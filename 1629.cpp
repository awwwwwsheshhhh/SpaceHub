...
int32_t main()
{
	int n; cin>>n; int k=n; vpi start_end; int ans=0;
	while(n--)
	{
		int a,b; cin>>a>>b; start_end.pb({b,a});
	}
	vs(start_end);int start_time=start_end[0].se;
	for(int i=0;i<k;i++)
	{
		if(start_end[i].se>=start_time){ans++;start_time=start_end[i].fi;}
	}
	cout<<ans<<endl;
}